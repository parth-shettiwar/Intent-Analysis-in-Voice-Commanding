...
sw_list = {"next","Next","to","on","through","move","an","un"}
all_stopwords_gensim = STOPWORDS.union(set(['insert','Insert','inserting','sentence','word','last','kindly','can','you','Can','Kindly','that','selected','text','please','Please','please?'])).difference(sw_list)
# all_stopwords_gensim = STOPWORDS.difference(sw_list)

data_path = os.getcwd() + '/data'
train_path = os.path.join(data_path, "final3.csv")
...
# for i in range(26):
#     os.mkdir(os.path.abspath(os.getcwd())+"/train/%s" %(str(i+1)))
for i in range(len(labels)):
    labels[i] = int(labels[i])
    input_text_lo[i] = input_text[i].lower()
    if not os.path.exists(os.getcwd()+"/train/%s" %(str(labels[i]+1))):
        os.makedirs(os.getcwd()+"/train/%s" %(str(labels[i]+1)))
        print(os.getcwd()+"/train/%s" %(str(labels[i]+1)))
    # print(os.path.abspath(os.getcwd()))
    with open(os.path.abspath(os.getcwd())+"/train/%s/%s.txt" %(str(labels[i]+1),str(i)),'w') as fp:
        # print(os.path.abspath(os.getcwd())+"/%s.txt" %(str(i)))
        print(input_text_lo[i], file=fp)
        print(input_text_lo[i])
        print(i)
        # print(os.path.abspath(os.getcwd())+"/train/%s/%s.txt" %(str(labels[i]+1),str(i)))
