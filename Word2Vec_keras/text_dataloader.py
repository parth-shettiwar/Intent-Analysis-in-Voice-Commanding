...
from exx.tensorflow_examples.lite.model_maker.core.data_util import dataloader
...
class TextClassifierDataLoader(dataloader.DataLoader):
  ...
  def from_folder(cls,
    ...
    idss=cls._save_data(examples, model_spec, label_names, tfrecord_file,
                   meta_data_file, vocab_file, is_training)

    # Loads data from cache directory.
    return cls._load_data(tfrecord_file, meta_data_file, model_spec),idss
  ...
  def _save_data(cls, examples, model_spec, label_names, tfrecord_file,
    ...
    idss=model_spec.convert_examples_to_features(examples, tfrecord_file,
                                            label_names,is_training)
