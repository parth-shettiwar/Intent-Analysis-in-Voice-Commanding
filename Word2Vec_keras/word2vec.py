...
class AverageWordVecModelSpec(object):
  ...
  def convert_examples_to_features(self, examples, tfrecord_file, label_names,is_training):
    ...
    for example in examples:
      ...
      input_ids = self.preprocess(example.text_a,is_training)
  ...
  def run_classifier(self, train_input_fn,k, validation_input_fn, epochs,
                     steps_per_epoch, validation_steps, num_classes):
    
    if epochs is None:
      epochs = self.default_training_epochs
    
    inputtt=tf.keras.layers.InputLayer(input_shape=[self.seq_len])
    embedding = tf.keras.layers.Embedding(len(self.vocab), self.wordvec_dim, input_length=self.seq_len)
    ...
    # k=k.tolist() 
    for jj in k:                 
      layer_output = get_3rd_layer_output([k])[0]
    # layer_output=layer_output.numpy()
    np.savetxt('temp.txt', layer_output)
    # X_embedded = TSNE(n_components=16,method='exact').fit_transform(layer_output)
    # x=X_embedded[:,0]
    # y=X_embedded[:,1]
  ...
  def preprocess(self, raw_text,is_training):
    """Preprocess the text for text classification."""
    tokens = self._tokenize(raw_text)
    if not is_training:
      self.load_vocab("/home/parth/Skill based LU/Word2Vec_keras/vocab")
    # print(self.vocab)
